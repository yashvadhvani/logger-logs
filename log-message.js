 ...
 * @param {string} insertEnclosingClass
 * @param {string} insertEnclosingFunction
...
/**
 * This function logs the specific line
 ...
 * @since 1.2
 */
function detectAll(document, tabSize, logMessagePrefix, loggerName) {
  const documentNbrOfLines = document.lineCount;
  const logMessages = [];
  for (let i = 0; i < documentNbrOfLines; i++) {
    const loggerLogMessage = new RegExp(`('|"|\`)${logMessagePrefix}.*`);
    if (loggerLogMessage.test(document.lineAt(i).text)) {
      const logMessageLines = { spaces: 0, lines: [] };
      for (let j = i; j >= 0; j--) {
        let numberOfOpenParenthesis = 0;
        let numberOfCloseParenthesis = 0;
        const infoRegex = RegExp(`/${loggerName}\.info/`);
        const errorRegex = RegExp(`/${loggerName}\.error/`);
        const warnRegex = RegExp(`/${loggerName}\.warn/`);
        const logRegex = RegExp(/console\.log/);
        if (
          infoRegex.test(document.lineAt(j).text) ||
          errorRegex.test(document.lineAt(j).text) ||
          warnRegex.test(document.lineAt(j).text) ||
          logRegex.test(document.lineAt(j).text)
        ) {
