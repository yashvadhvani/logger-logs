...
function checkObjectDeclaration(lineCode) {
  ...
  return objectRegex.test(lineCode);
}

/**
 * Return
 * @param {string} lineCodeSelectionLine
 * @param {string} lineCodeSelectionNextLine
 * @return {boolean}
 */
function checkArrayDeclaration(
  lineCodeSelectionLine,
  lineCodeSelectionNextLine
) {
  const arrayDeclarationRejex = /(const|let|var)?(\s*)[a-zA-Z0-9]*(\s*)=(\s*)\[/;
  return (
    arrayDeclarationRejex.test(lineCodeSelectionLine) ||
...
/**
 * Check if the lineCode contains function definition
 * @param {string} lineCode
 * @return {boolean}
 */
